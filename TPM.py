...
class DatabasePreparation(object):
    ...
    def preprocessing(self):
        self.data.drop(['date_visit'], axis = 1) 
    ...
    def locationsDatabase(self, locations):
    ...
    def tripsDatabase(self, trips_profiles, classification):
        self.list_trips = {}
        for i in trips_profiles.items():
            for j in i[1]:
...
class TripsDatabase(object):
    ...
    def database_voyages(self, liste):
    ...
    def run(self, liste):
        self.database_voyages(liste)
        #self.embedding_PCA(date, region)

class HierarchicalClustering(object):

    def hierarchical_clustering(self, database, k):
...
class KMeansClustering(object):
    ...
    def kmeans_clustering(self, database, k):
        modelkmeans = KMeans(n_clusters = k, init = 'k-means++', n_init = 100)
        modelkmeans.fit_predict(database)
        return modelkmeans.labels_

class LDAClustering(object):

    def kmeans_clustering(self, date, region):
        Sum_of_squared_distances = []
        K = np.arange(2,30)
        for k in K:
            km = KMeans(n_clusters=k, max_iter=200, n_init=10)
            km = km.fit(self.train)
            Sum_of_squared_distances.append(km.inertia_)

        elbow = pd.DataFrame(np.column_stack([K, Sum_of_squared_distances]), columns = ["K", "Sum_of_squared_distances"])
        fig = px.scatter(elbow, x="K", y="Sum_of_squared_distances")
        fig.update_traces(mode="markers+lines")
        fig.show()
        fig.write_html("res/" + region + "/" + str(date) + "/elbow_kmeans.html")
        self.true_k = int(input("Quel est le nombre de clusters ?"))

        modelkmeans = KMeans(n_clusters = self.true_k, init = 'k-means++', n_init = 100)
        modelkmeans.fit(self.train)
        kmeans_labels = modelkmeans.labels_
        self.train['label'] = kmeans_labels
        print(self.train.head())
        self.visualization_PCA(date, region, "kmeans", self.train['label'].values)

class KPrototypesClustering(object):

    def kprototypes_clustering(self, database, k):
...
class TouristProfileMeasure(object):
    ...
    def distanceMatrix(self, list_trips, a, b, distance_classification, distance_season):
        ...
        for i in range(0, len(list_trips) - 1):
            ...
            resA = self.classificationDistance(distance_classification, tabA, idA)
            resB = self.seasonDistance(distance_season, tabB, idB)
    ...
    def entanglementMatrix(self, list_trips, distance_classification, distance_season):
        ...
        for score_a in range(0, 11): 
            ...
            self.distanceMatrix(list_trips, score_a, score_b, distance_classification, distance_season)
    ...
    def entanglementVisualization(self, distance_classification, distance_season, date, region):
        ...
        fig.show()
        fig.write_html("res/" + region + "/" + str(date) + "/" + distance_classification + "_" + distance_season + "_" + "entanglement.html")

    def matrixProjection(self, a, b, distance_classification, distance_season, date, region):
        ...
        fig.update_traces(mode="markers")
        fig.show()
        fig.write_html("res/" + region + "/" + str(date) + "/" + str(a) + "_" + str(b) + "_" + distance_classification + "_" + distance_season + "_" + "projection.html")

    def classificationDistance(self, distance, tab, id):
        res = None
    ...
    def seasonDistance(self, distance, tab, id):
    ...
    def run(self, list_trips, a, b, distance_classification, distance_season, date, region, k):
        self.distanceMatrix(list_trips, a, b, distance_classification, distance_season)
        self.matrixProjection(a, b, distance_classification, distance_season, date, region)
        self.hierarchicalClustering(k)
        return self.r_groups
    
    def entanglement(self, list_trips, distance_classification, distance_season, date, region):
        self.entanglementMatrix(list_trips, distance_classification, distance_season)
        self.entanglementVisualization(distance_classification, distance_season, date, region)

class ClusteringMeasures(object):

    def numberClusters(self, a, b, distance_classification, distance_season, date, region, database):
        ...
        for i in range(0,r_length[0]):
            ...
            r_nb = r_NbClust.NbClust(data = r_database, method = "ward.D2", distance = "euclidean", min_nc = min, max_nc = max, index = r_index[i])
        ...
        fig.update_traces(mode="markers+lines")
        fig.write_html("res/" + region + "/" + str(date) + "/" + str(a) + "_" + str(b) + "_" + distance_classification + "_" + distance_season + "_" + "silhouette.html")
        fig.show()
...
class TPMAlgorithm(object):
    ...
    def saveResults(self, a, b, distance_classification, distance_season, date, region):
        path = "res/" + region + "/" + str(date) + "/"        
        file = str(a) + "_" + str(b) + "_" + distance_classification + "_" + distance_season
        #file = "Hierarchical"
    ...
    def convertSummaryToCsv(self, a, b, distance_classification, distance_season, date, region):
        csv_columns = list(self.summary[1].keys())
        path = "es/" + region + "/" + str(date) + "/"
        file = str(a) + "_" + str(b) + "_" + distance_classification + "_" + distance_season + "_clusters.csv"
        #file = "Hierarchical_clusters.csv"
        Path(path).mkdir(parents=True, exist_ok=True)
        csv_file = path + "/" + file
    ...
    def convertCsvToExcel(self, a, b, distance_classification, distance_season, date, region):
        path = "res/" + region + "/" + str(date) + "/"
        file = str(a) + "_" + str(b) + "_" + distance_classification + "_" + distance_season + "_clusters.csv"
        #file = "Hierarchical_clusters.csv"
        ...
        path = "res/" + region + "/" + str(date) + "/"
        file = str(a) + "_" + str(b) + "_" + distance_classification + "_" + distance_season + "_clusters.xlsx"
    ...
    def run(self, list_trips, static, a, b, distance_classification, distance_season, date, region, r_groups):
        self.extractionClusters(r_groups)
        self.informationMerge(list_trips, static)
        self.summaryClusters()
        self.saveResults(a, b, distance_classification, distance_season, date, region)
        self.convertSummaryToCsv(a, b, distance_classification, distance_season, date, region)
        self.convertCsvToExcel(a, b, distance_classification, distance_season, date, region)
